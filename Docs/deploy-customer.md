...
| 2026-09-18 | prod | §3.2 SQL fallback (`prod` is reserved in `create-organization`, so the admin-app path refuses it), §3.3 SQL fallback for the superuser's admin membership, §2 curl check, sign-in card verified at `https://prod.handytally.com`; updated `create-organization` copied to the host | Lucas, commands prepared by Claude | HT-41. Closes the "every command run once" item from the 2026-09-16 rows: §3.2 and §3.3 have now been run for a fresh organisation. Check query showed `wgelectricus` with 0 active memberships (its users sign in as members of no organisation or as the superuser); worth a look before the next customer invite |
| 2026-09-19 | wgelectricus | §3.6 steps 4–6: prod ran `v1.5.0`; *Promote release to customer* run 35413297679 created Worker `handytally-web-wgelectricus` on `wgelectricus.handytally.com/*` from the Release asset; host reports `v1.5.0`, footer `v1.5.0 · wgelectricus`; §4 smoke test passed | Lucas | HT-41. Steps 1–3 were done in PR #101 (the `env.wgelectricus` block shipped with the pipeline). First customer host pinned to a release; from now on it only moves via *Promote* |
