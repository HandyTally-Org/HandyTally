...
| [release-process.md](release-process.md) | How a change reaches customers: demo → prod → customer hosts, cutting a release (tag, GitHub Release, promote), the release PR checklist, customer notice, smoke test, rollback, hotfix, and the expand-only rule for the shared database | **Yes** — fix the step in the same PR that changes a workflow |
| [deploy-customer.md](deploy-customer.md) | Runbook for bringing a customer live on `<subdomain>.handytally.com`: prerequisites, per-customer steps (including their own Worker environment), smoke test, rollback, troubleshooting | **Yes** — fix the step in the same PR that changes the behaviour |
...
- [`../CHANGELOG.md`](../CHANGELOG.md) — technical changelog (ticket numbers, migrations)
- [`../RELEASE_NOTES.md`](../RELEASE_NOTES.md) — customer-facing release notes, rendered in the app at `/whats-new`
