...
**Current state:** the app now selects and stamps the organisation per host, but reads are not yet filtered by `organization_id` and, per the HT-14 analysis, the pre-existing rows were moved to the `demo` organisation and the company profile to `wgelectric` by HT-40 (`20260917130000_wgelectric_and_demo_organizations.sql`); the five test organisations are inactive. Tenant isolation holds only once the wide `authenticated` policies are replaced by the org-scoped ones. See [DI-02](dev-issues.md#di-02).
