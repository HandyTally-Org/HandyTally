...
CREATE OR REPLACE FUNCTION public.create_organization_atomic(org_name text, org_subdomain text, created_by uuid)
 RETURNS TABLE(id uuid, name text, subdomain text, status organization_status)
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
DECLARE
...
BEGIN
  ...
  IF length(trim(org_name)) < 2 OR length(trim(org_name)) > 100 THEN
  ...
  IF length(org_subdomain) < 3 OR length(org_subdomain) > 63 THEN
    RAISE EXCEPTION 'Subdomain must be between 3 and 63 characters';
  END IF;
  
  -- Check subdomain format
  IF NOT org_subdomain ~ '^[a-z0-9]([a-z0-9-]*[a-z0-9])?$' THEN
    RAISE EXCEPTION 'Invalid subdomain format';
  END IF;
  
  -- Check for reserved subdomains
  IF org_subdomain IN ('www', 'api', 'admin', 'app', 'mail', 'ftp', 'localhost', 'staging', 'test', 'dev', 'demo', 'beta', 'alpha', 'prod', 'production', 'development') THEN
    RAISE EXCEPTION 'Subdomain is reserved';
  END IF;

  -- Start transaction
  BEGIN
    -- Check if subdomain already exists (with row lock)
    PERFORM id FROM organizations WHERE subdomain = org_subdomain FOR UPDATE;
    
    IF FOUND THEN
      RAISE EXCEPTION 'Subdomain already exists' USING ERRCODE = '23505';
    END IF;
    
    -- Create organization
    INSERT INTO organizations (name, subdomain, status, created_at, updated_at)
    VALUES (trim(org_name), org_subdomain, 'pending', NOW(), NOW())
    RETURNING organizations.id INTO new_org_id;
    
    -- Return the created organization
    RETURN QUERY
    SELECT organizations.id, organizations.name, organizations.subdomain, organizations.status
    FROM organizations
