...
### Added
- **HT-25** — **Admin › Export** page with an *Export to → QuickBooks Online* target. **Customers** downloads `quickbooks-customers.xlsx` in Intuit's 16-column layout (text ZIP, org-level Country, optional opening balance from open invoices); **Invoices** downloads `quickbooks-invoices.csv` (one row per line item, header values on the first row, recomputed `*ItemAmount`, a synthetic *Service Fee* line, per-line `Taxable`/`TaxRate`, `Terms` from the stored terms or the date gap, single-line `Memo`, `Service Date` from the job) split into files of at most 1,000 rows without splitting an invoice. Estimates, work orders and cancelled documents are never exported. The export stops on duplicate client names or invoice numbers and lists them; it warns about recomputed amounts, totals that will not match, and paid invoices (QBO imports every invoice as open). Export settings (country, default terms, date format) are edited on the page and stored in `organization_settings.export`. Rules in `web/utils/quickbooksExport.ts`; nav key `admin-export`.
- **HT-25** — Groundwork for the QuickBooks Online export. Clients gain **Company**, **Mobile** and **Website**, and the ZIP is stored as text in a new `clients.postal_code` column so `02134` and ZIP+4 survive; the old numeric `clients.zip` stays for the compatibility window and a trigger (`clients_sync_postal_code`) keeps the two in step in both directions. Invoices gain **Terms** (Due on receipt / Net 15 / 30 / 60, `invoices.terms`), which sets the due date and defaults from the organisation's export settings; each line item gains a **Taxable** checkbox (`invoice_items.taxable`, default true) and the tax amount now applies only to taxable lines plus the fee. New `organization_settings.export` document (country, default terms, date format). Migration `20260919100000_quickbooks_export_fields.sql`, expand-only.
